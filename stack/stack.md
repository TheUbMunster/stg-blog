...
since Alan Turing's studies[^0 CORROBORATE ME].

In this post I'll be talking about how the stack works.
...
that function, and "adopt"[^7] it into the stack of the calling function.
...
comes to our (e.g) ABI designs, hardware architecture designs, or paradigms that evolved over time. Many of modern
efficiency gains seem to come from making hardware more complicated.

One example of this is the TLB. The TLB is a fairly recent invention, but it allows for repeated memory translations
(virt->phys) to happen faster, which does result in real benefits. An even more recent advancement allowed line entries
to persist across context switching, allowing these cached calculations to possibly survive while processes are getting
their turn running on the CPU (but again, adding this functionality makes things faster *& more complicated*). CPU's
were certainly simpler without these things, but we concess simplicity for speed. If only electricity moved faster,
maybe stuff like this wouldn't be necessary!
